...
// const ComperssionWebpackPlugin = require('compression-webpack-plugin') //文件压缩成gzip的依赖

// let plugins
// if (process.env.NODE_ENV === 'production') {
//     plugins = [
//         new ComperssionWebpackPlugin({
//             filename: '[path].gz[query]', //目标文件
//             algorithm: 'gzip', //使用gzip压缩
//             test: /\.(js|css|html|svg)$/, //使用正则来筛选需要压缩的文件类型
//             threshold: 10240, //资源文件大于10240（10kb）时文件会被压缩
...
// }
// 此文件为可选配置项，如果存在这个文件，脚手架会自动加载
// 此文件导出一个包含选项的对象
module.exports = {
    //publicPath:基本路径，当为./时所有资源都会被链接为相对路径；
    //publicPath:'./'
    //开发过程中架设在根路径，可以用条件式的值进行判断为开发（development）还是生产(production)环境
    //NODE_ENV在package中的脚本库中设置，项目启动时会和启动命令行结合，（https://www.cnblogs.com/usebtf/p/9912413.html）此链接详细讲解了process.env.NODE_ENV的作用
    ...
    //是否保存的时候使用’eslint-loader‘进行检查，为error时检查出的错误会触发编译失败
    lintOnSave: true,
